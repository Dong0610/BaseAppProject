...
fun FragmentActivity.colorRes(@ColorRes color: Int): Int = ContextCompat.getColor(this, color)
fun Fragment.colorRes(@ColorRes color: Int): Int =
    activity?.let { ContextCompat.getColor(it, color) } ?: "#00000000".toColorInt()
...
fun String.toColorOpacity(@IntRange(0, 100) opacity: Int): Int {
    ...
    val alphaHex = alphaMap[safeOpacity]
    val cleanColor = this.removePrefix("#")
...
fun String.isValidHexColor(): Boolean {
    ...
    return this.replace("#", "").matches(hexPattern)
}
